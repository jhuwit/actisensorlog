---
output: github_document
---

<!-- README.md is generated from README.Rmd. Please edit that file -->
<!-- badges: start -->
[![R-CMD-check](https://github.com/jhuwit/actisensorlog/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/jhuwit/actisensorlog/actions/workflows/R-CMD-check.yaml)
[![Codecov test coverage](https://codecov.io/gh/jhuwit/actisensorlog/branch/main/graph/badge.svg)](https://app.codecov.io/gh/jhuwit/actisensorlog?branch=main)
<!-- badges: end -->

```{r setup, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/README-",
  cache = TRUE
)
```

# actisensorlog

`actisensorlog` processes [SensorLog](http://sensorlog.berndthomas.net/) and [SensorLogger](https://www.tszheichoi.com/sensorlogger) exports. The package
focuses on timestamp normalization, distance calculations, duplicate-time
checks, and daily summaries.

Core entry points:

- `acti_process_sensorlog()` for duplicate checking, home-distance
  calculations, and timezone handling
- `acti_sensorlog_process_time()` for parsing SensorLog timestamps
- `acti_minute_sensorlog()` and `acti_summarize_sensorlog()` for minute and
  daily summaries

## Installation

You can install `actisensorlog` from GitHub with:

```{r gh-installation, eval = FALSE}
# install.packages("remotes")
remotes::install_github("jhuwit/actisensorlog")
```

## Quick Start

```{r sensorlog-example}
library(actisensorlog)

sensorlog <- suppressMessages(
  actiread::acti_read_sensorlog(actiread::acti_example_sensorlog_file())
)

processed_sensorlog <- acti_process_sensorlog(
  sensorlog,
  lat = 39.3,
  lon = -76.6,
  expected_timezone = "America/New_York",
  check_data = FALSE
)

minute_sensorlog <- acti_minute_sensorlog(processed_sensorlog)
summary_sensorlog <- acti_summarize_sensorlog(processed_sensorlog)
```

The SensorLog example gains `distance`, `distance_traveled`, and
`is_within_home` fields:

```{r sensorlog-processed}
head(processed_sensorlog)
```

The minute-level view fills missing minutes between observations:

```{r sensorlog-minute}
head(minute_sensorlog)
```

The daily summary collapses the data to a single row per date:

```{r sensorlog-summary}
summary_sensorlog
```

```{r sensorlogger-example}
sensorlogger <- suppressMessages(
  actiread::acti_read_sensorlogger_location(
    actiread::acti_example_sensorlogger_location_file()
  )
)

processed_sensorlogger <- acti_process_sensorlog(
  sensorlogger,
  lat = 39.3,
  lon = -76.6,
  expected_timezone = "America/New_York"
)

minute_sensorlogger <- acti_minute_sensorlog(processed_sensorlogger)
summary_sensorlogger <- acti_summarize_sensorlog(processed_sensorlogger)
```

The packaged SensorLogger location example follows the same flow:

```{r sensorlogger-processed}
head(processed_sensorlogger)
```

```{r sensorlogger-minute}
head(minute_sensorlogger)
```

```{r sensorlogger-summary}
summary_sensorlogger
```
